...
let draw_grid (x0, y0, grid_size, cell_size : int * int * int * int) : unit =
  for i = 0 to grid_size do
    let x : int = x0 + i * cell_size in
    moveto x y0;
    lineto x (y0 + grid_size * cell_size)
  done;
  for j = 0 to grid_size do
    let y : int = y0 + j * cell_size in
    moveto x0 y;
...
let rec positions_list (pos, dir, length : (int * int) * int * int) : (int * int) list =
  if length = 0 then
    ...
    let (x, y) : int * int = pos in
    let next_pos : int * int =
      if dir = 0 then (x + 1, y)
...
let rec can_place_ship (grid, positions : t_grid * (int * int) list) : bool =
  if positions = [] then
    ...
    let (x, y) : int * int = List.hd positions in
    ...
    if x < 0 || x >= n || y < 0 || y >= n then
      ...
      can_place_ship grid (List.tl positions)
;;
(**
Commentaire :
...
let rec auto_placing_ships (grid, ships : t_grid * (string * int) list) : t_ship list =
  if ships = [] then
    ...
    let (name, len) : string * int = List.hd ships in
    let rec try_place ( ) : (int * int) list =
      let n : int = Array.length grid in
      let x : int = Random.int n in
      let y : int = Random.int n in
      let dir : int = Random.int 4 in
      let pos_list : (int * int) list = positions_list (x, y) dir len in
      if can_place_ship grid pos_list then pos_list else try_place ()
    in
    let pos : (int * int) list = try_place () in
    let () : unit = place_ship_on_grid grid pos in
...
let color_cell (x0, y0, i, j, cell_size, col : int * int * int * int * int * Graphics.color) : unit =
  set_color col;
  let (px, py) : int * int = cell_to_pixel x0 y0 i j cell_size in
...
(**
Commentaire :
display_grid_color parcourt la grille et colore les cases en fonction de leur état :
...
let battleship_game () : unit =
  ...
  let game : t_battleship = init_battleship params in
  ...
  moveto x_player (y_grid + params.grid_size * params.cell_size + 10);
  draw_string "Joueur";
  (* Début de l'alternance des tirs *)
  all_shoot game.comp_grid game.comp_ships game.player_grid game.player_ships params;
