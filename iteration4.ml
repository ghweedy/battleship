...
let init_battleship (params : t_params) : t_battleship =
  ...
  (* Pour tester les tirs, on place aussi automatiquement les bateaux sur la grille du joueur *)
  let player_ships = auto_placing_ships (player_grid, params.ship_sizes) in
  { comp_grid = comp_grid;
...
let battleship_game () : unit =
  ...
  draw_string "Joueur";
  player_turns game.player_grid game.player_ships params 5;
  let key = read_key () in
  close_graph ()
;;
(**
Commentaire :
La fonction principale battleship_game orchestre l'exécution de l'itération 4.
Elle effectue les opérations suivantes :
  - Initialise les paramètres et l'état du jeu via init_params et init_battleship.
  - Calcule les dimensions de la fenêtre (ici 390×270 pixels) et l'ouvre.
  - Affiche la grille du joueur avec les bateaux placés.
  - Permet au joueur d'effectuer 5 tirs successifs en mettant à jour la grille
    (tir manqué, touché, coulé) grâce à player_turns.
  - Attend une saisie clavier puis ferme l'affichage.
@author TOTSKYI Hlib
@author TERRENOIRE Yvan
@author AHAMADI Izaki
*)
